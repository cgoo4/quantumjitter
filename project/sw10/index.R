...
data_df <- sparql_select(endpoint, query)

data_df <- data_df |>
  mutate(
    trans_date = as_date(ppd_transactionDate),
    ...
    prop_type = replace_values(prop_type, "otherPropertyType" ~ "Other")
  ) |>
...
qtile_df <-
  ...
  reframe(
    price = quantile(amount, c(0.25, 0.5, 0.75)),
...
p2 <- qtile_df |>
  ...
  annotate(
    "text",
    x = yearquarter("2016 Q3"),
    y = 130,
    label = "Brexit\nVote",
    size = 3
  ) +
  scale_x_yearquarter(date_breaks = "2 years") +
  labs(x = NULL, y = "Transactions", caption = "Source: HM Land Registry") +
...
data_df |>
  ggplot() +
  geom_mosaic(
    aes(product(prop_type, est_type), fill = prop_type),
    offset = 0.02,
    divider = mosaic("h")
  ) +
