...
Setting `approximation` to `FALSE` means `auto.arima` will explore a wider range of potential models. For *G-Cloud* this works as before. The much shorter *DOS* series, however, is now rejected by {fable} as "a simple polynomial" and unsuitable for the automatic search, so it's pinned to an ARIMA(0,1,1) with drift, which continues the series' recent trajectory.
...
The G-Cloud framework comprises three lots: Cloud Hosting, Cloud Software and Cloud Support.
