...
-   `, London, SW10 .+$` finds for example `, London, SW10 0ST` and all other `SW10` postcodes. See the difference by comparing the data frame `band_df` ( before) with `band_df2` (after).
-   `(1ST|2ND|3RD|4TH|5TH|6TH) FLR` does the same for floor numbers irrespective of whether it's the 1ST or 2ND etc.
-   `(?<=COURT|WALK).*` uses a [positive lookbehind](https://www.rexegg.com/regex-lookarounds.html) to find anything following the word `COURT` or `WALK` (since the postcode is stored in a separate variable and already isolates the road, the road name is redundant).
-   `^([0-9]{1,3})([A-Z])(?= .*)` and `\\2 \\1` makes use of [capture groups](https://www.rexegg.com/regex-capture.html) to temporarily memorise a number (capture group 1) followed by a letter (capture group 2), then swap them around. This is because a flat number may be `C22` in one dataset and `22C` in the other.
